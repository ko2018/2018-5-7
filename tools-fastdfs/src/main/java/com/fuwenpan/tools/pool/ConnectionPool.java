...
public class ConnectionPool {
	...
	public static synchronized ConnectionPool getInstance() {
		if (connectionPool == null) {
			...
			try {
				...
				int minPoolSize = iniReader.getIntValue("minPoolSize", 10);
				int maxPoolSize = iniReader.getIntValue("maxPoolSize", 100);
				int waitTimes = iniReader.getIntValue("waitTimes", 200);
				...
				connectionPool = new ConnectionPool(10, 100, 200);
			}
		}
	...
	public void createTrackerServer(String logId, int flag) {
		...
		try {
			...
			boolean isSocket = org.csource.fastdfs.ProtoCommon
			...
			if (trackerServer != null) {
				try {
					...
					LOGGER.error(
							"[创建TrackerServer(createTrackerServer)--关闭trackerServer异常]["
									+ logId + "][异常：{}]", e);
				}
			}
	...
	private void initClientGlobal() throws Exception {
		String classPath = new File(ConnectionPool.class.getResource("/")
				.getFile()).getCanonicalPath();
		String fdfsClientConfigFilePath = classPath + File.separator
				+ "fdfs_client1.conf";
