...
public class ${table.classDomain}ServiceImpl extends BaseServiceImpl<${table.classDomain}> implements ${table.classDomain}Service {
	...
	@CacheSpeObject(value = "${table.classDomainVar}", key = "#${table.classDomainVar}.<#list table.columnList as item><#if item.pk =='Y'>${item.propertyName}</#if></#list>")
	@Override
	...
	public PageResult<${table.classDomain}Dto> pageListDto(PageObject pageObject) {
		long totalCount = this.${table.classDomainVar}Dao.pageCount(pageObject);
