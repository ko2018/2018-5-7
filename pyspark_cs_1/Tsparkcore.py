...
def createSparkRDD(sc, table):
    ...
    "hbase.zookeeper.quorum": "192.168.6.89,192.168.6.90,192.168.6.88",
     "hbase.mapreduce.scan.row.start": "000",
     "hbase.mapreduce.scan.row.stop": "001"
    }
...
def map_std_Row_Two_Way_anova(colKey, option_list):
    cols = colKey['row'].split(":")
    vals = colKey['value']
    hasType = None
    if vals is not None:
...
def map_std_Row_test(colKey, option_list):
    cols = colKey['row'].split(":")
    vals = colKey['value']
    valsDict_rs = {}
    if vals is not None:
